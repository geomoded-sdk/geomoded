...
// Deprecated mods are last by default
...
if (aIsDeprecated != bIsDeprecated) {
    return bIsDeprecated; // o que for deprecated vai pro final
}

// Outdated mods are first by default
...
if (aIsOutdated != bIsOutdated) {
    return aIsOutdated; // o que for outdated vai pro topo
}
        }
...
bool InstalledModsQuery::preCheck(ModSource const& src) const {
    // Invalid .geomoded files are collected up into one entry instead of
            ...
            content.mods.push_back(SpecialModListItemSource {
                ...
                .onDetails = [=]() {
                    createQuickPopup(
                        "Invalid mods",
                        fmt::format(
                            "You have <cy>{}</c> invalid <ca>.geomoded</c> files in your "
